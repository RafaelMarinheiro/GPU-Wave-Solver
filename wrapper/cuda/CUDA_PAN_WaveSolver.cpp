...
CUDA_PAN_WaveSolver::CUDA_PAN_WaveSolverSolver(REAL timeStep,
...
CUDA_PAN_WaveSolver::~CUDA_PAN_WaveSolverSolver(){
...
const Tuple3i & CUDA_PAN_WaveSolver::fieldDivisions() const{
	...
	return Tuple3i(nx, ny, nz);
}

const vector<const TriMesh *> & CUDA_PAN_WaveSolver::meshes() const{
	return this->_meshes;
}
...
void CUDA_PAN_WaveSolver::vertexPressure(const Tuple3i & index,
	...
	int nx, ny, nz;
	wave_sim_get_divisions(this->wave, &nx, &ny, &nz);
	Number_t * u = wave_sim_get_u(this->wave);
	int stride = 4*nx*ny*nz;
	int pos = 4*(index[0] + nx*(index[1] + ny*index[2]));

	pressure[0] = (REAL) u[pos + 0*stride];
	pressure[1] = (REAL) u[pos + 1*stride];
	pressure[2] = (REAL) u[pos + 2*stride];
	pressure[3] = (REAL) u[pos + 3*stride];
	pressure[4] = (REAL) u[pos + 4*stride];
	pressure[5] = (REAL) u[pos + 5*stride];
}
